...
Pattern-based SAST tools such as Bandit~\cite{bandit} and Semgrep~\cite{semgrep}
match syntactic or lightly-semantic patterns against source and are the de-facto
standard for Python in CI because they are fast and require no build. Dataflow-based
analysers---Pixy~\cite{pixy} for PHP, FlowDroid~\cite{flowdroid} for Android, and
commercial engines such as CodeQL~\cite{codeql}---track tainted values from sources
to sinks and are more precise on injection classes but heavier to deploy. Q-Trace
occupies a deliberate middle ground: it performs targeted, interprocedural taint and
symbolic reachability only where a stealth pattern demands it, and falls back to
fast abstract-syntax-tree (AST) rules elsewhere.
...
standard~\cite{nielson}. Q-Trace uses a lightweight static taint pass to connect
...
input-space exploration to expose it. Environmental keying and sandbox evasion are
catalogued by MITRE ATT\&CK as T1480.001 and T1497 respectively~\cite{attack}.
Q-Trace targets the \emph{static} footprint of these techniques: the presence of a
random/stateful guard around a sink, or of an anti-analysis primitive, is itself the
signal.
...
Q-Trace combines Shannon entropy~\cite{shannon} with the Higuchi fractal
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.86\linewidth]{fig1_arch.pdf}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.82\linewidth]{figA_confflow.pdf}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.6\linewidth]{fig5_confidence.pdf}
...
gate of~\eqref{eq:gate}. Fig.~\ref{fig:ablation} reports the result. The severity-only
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.5\linewidth]{fig6_ablation.pdf}
\caption{Confidence-axis ablation. Removing the confidence axis (severity-only gate)
recovers one detection but introduces two false positives (5.9\pct{} FP). The
two-axis gate trades 3.2 points of gate-recall for the elimination of \emph{all}
false positives---the trade that governs real-world adoption.}
\label{fig:ablation}
\end{figure}
...
Fig.~\ref{fig:refine} quantifies the change against the immediately-prior version of
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.55\linewidth]{fig12_refine.pdf}
\caption{Effect of the evidence-based anti-analysis refinement. The refined engine
improves gate-recall and $F_1$ while removing false positives and adding
...
Figs.~\ref{fig:recall}--\ref{fig:f1} break out recall, false-positive rate, and $F_1$
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.6\linewidth]{fig2_quadrant.pdf}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.6\linewidth]{fig3_recall.pdf}\\[2pt]
\includegraphics[width=0.6\linewidth]{fig4_fp.pdf}
\caption{Malware recall (top) and false-positive rate (bottom) by tool at each tool's
CI gate. Q-Trace holds 0\pct{} FP; the false positives raised by Bandit and Semgrep are
precisely the everyday-code patterns that drive alert fatigue.}
\label{fig:recall}
\end{figure}

\begin{figure}[!t]
\centering
\includegraphics[width=0.55\linewidth]{fig10_f1.pdf}
\caption{$F_1$ on the shared code corpus. The gap is driven jointly by higher recall
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.52\linewidth]{fig7_confusion.pdf}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.66\linewidth]{fig8_matrix.pdf}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.6\linewidth]{fig11_onlyq.pdf}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.62\linewidth]{fig9_latency.pdf}
...
non-gating rather than break the build. The ablation of Fig.~\ref{fig:ablation}
